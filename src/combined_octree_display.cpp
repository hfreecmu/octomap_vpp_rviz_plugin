#include "octomap_vpp_rviz_plugin/combined_octree_display.h"
...
CombinedOcTreeDisplay::CombinedOcTreeDisplay() : octomap_rviz_plugin::OccupancyGridDisplay()
{
...
void CombinedOcTreeDisplay::incomingMessageCallback(const octomap_msgs::OctomapConstPtr& msg)
{
  ++messages_received_;
  ...
  // creating octree
  octomap_vpp::CombinedOcTree* octomap = NULL;
  octomap::AbstractOcTree* tree = octomap_msgs::msgToMap(*msg);
  if (tree){
    octomap = dynamic_cast<octomap_vpp::CombinedOcTree*>(tree);
    if(!octomap){
      setStatusStd(StatusProperty::Error, "Message", "Wrong octomap type. Use a different display type.");
      return;
    }
  ...
  size_t pointCount = 0;
  {
    ...
    for (octomap_vpp::CombinedOcTree::iterator it = octomap->begin(treeDepth), end = octomap->end(); it != end; ++it)
    {
            ...
            for (unsigned int idxCase = 0; idxCase < 3; ++idxCase)
            {
              ...
              for (int i = 0; allNeighborsFound && i < 2; ++i)
              {
                ...
                for (key[idx_1] = nKey[idx_1] + diff[0] + 1; allNeighborsFound && key[idx_1] < nKey[idx_1] + diff[1]; key[idx_1] += stepSize)
                {
                  ...
                  {
                    octomap_vpp::CombinedOcTreeNode* node = octomap->search(key, treeDepth);
...
void CombinedOcTreeDisplay::setVoxelColor(rviz::PointCloud::Point& newPoint, octomap_vpp::CombinedOcTreeNode &node, double minZ, double maxZ)
{
...
bool CombinedOcTreeDisplay::checkType(std::string type_id)
{
  if(type_id == "CombinedOcTree") return true;
  else return false;
}
...
PLUGINLIB_EXPORT_CLASS( octomap_vpp_rviz_plugin::CombinedOcTreeDisplay, rviz::Display)
